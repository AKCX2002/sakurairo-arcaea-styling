...
- `.arcaea-article-content h1` 使用 `background-clip: text` 的 SVG 纹理图片标题；不要用 `h1::before` 做装饰。
- Sakurairo 会用 `h1-h6::before` 做目录锚点占位。Arcaea 的 H2/H3/H4 视觉伪元素不能被末尾 guard 重新覆盖为 `height: 80px`。
- 文章正文壳层透明度需参考 Sakurairo 的更深玻璃层：正文 wrapper 应有约 `0.90+` 的深色背景、边框和轻 blur，避免只剩背景图、正文层次看不出来。
