...
6. **babel-arcaea-code CI/CD**：`Release & Sync` 的定时依赖更新不再创建 `chore/deps-bump` PR；有依赖变化时应在 `sync-deps` job 内直接提交到 `main`、递增插件版本、构建 zip 并创建 GitHub Release。仓库 Actions 权限必须保持 `default_workflow_permissions=write`，否则 `git push origin HEAD:main` 或 `gh release create` 会失败。
